...
class FeatureDisplayPopup(Popup):
    def __init__(self, **kwargs):
        ...
        self.feature_container = BoxLayout(orientation='vertical', spacing=10, size_hint_y=None)
        self.feature_container.bind(minimum_height=self.feature_container.setter('height'))
        self.scroll_view.add_widget(self.feature_container)
        
        self.content.add_widget(self.scroll_view)

    def update_content(self, features):
        self.feature_container.clear_widgets()
        
        for feature_name, image_path in features.items():
            # Add text label for the feature
            feature_label = Label(text=feature_name, font_size=14, size_hint_y=None, height=30)
            self.feature_container.add_widget(feature_label)
            
            # Load and add image if it exists
            if image_path:
                img = Image(source=image_path, size_hint_y=None, height=200)
                self.feature_container.add_widget(img)
        
        self.feature_container.height = sum(widget.height for widget in self.feature_container.children)
...
class CurrencyRecognitionApp(App):
    ...
    def process_features(self):
        if self.recognized_denomination:
            ...
            for label in feature_labels_to_check:
                # Construct path to feature image based on label
                image_path = os.path.join("path/to/feature_images/", f"{label}.png")
                if os.path.exists(image_path):
                    detected_features[label] = image_path
                else:
                    detected_features[label] = None  # No image found for this feature

            # Determine if the currency is "Real" or "Fake"
            if len(detected_features) > 5:
                self.detection_result_label.text = "Detection: Real"
            else:
                self.detection_result_label.text = "Detection: Fake"

            # Show the popup with detected features
            if detected_features:
                self.feature_display_popup.update_content(detected_features)  # Update and show the popup
                self.feature_display_popup.open()
            else:
                self.feature_display_popup.update_content({"No features detected": None})
                self.feature_display_popup.open()
